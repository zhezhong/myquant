...
class Mystrategy(StrategyBase):
    ...
    def on_bar(self, bar):
        #self.unit_volume = self.get_positions()[0].
        if self.init:
        ...
        if(len(AMA1)!=0 and len(AMA2)!=0):
            if AMA1[-1]>AMA2[-1]*((1+self.sma_threshold))  and self.long == False :
                self.long = True
                #self.long_price =
                self.open_long(self.symbol[:4], self.symbol[5:], price=0, volume=4000)
            if AMA1[-1] > AMA2[-1] and self.short:
                #  close short
                self.short = False
                self.long_price = self.price
                print('open long positions%s %s %d' % (self.symbol[:4], self.symbol[5:], 4000))

                # 强势上涨过程中不做空。
            if AMA1[-1] < AMA2[-1] and self.long :
                # 平 多仓
                self.long = False
                self.close_long(self.symbol[:4], self.symbol[5:], price=0, volume=4000)
            if AMA1[-1] < (AMA2[-1] * (1 - self.sma_threshold)) and self.short == False:
                # 开 空仓
                self.close_long(self.symbol[:4], self.symbol[5:], price=0, volume=4000)
                self.short = True
                self.short_price = self.price

                print('open short positions%s %s %d' % (self.symbol[:4], self.symbol[5:], 5000))
...
if __name__ == '__main__':
    ...
    myStrategy.backtest_config(
        start_time='2017-06-01 08:00:00',
