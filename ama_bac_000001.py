...
class Mystrategy(StrategyBase):
    ...
    def on_bar(self, bar):
        self.etime = bar.strendtime[:10]+' '+bar.strendtime[11:19]
        ...
        if(len(AMA1)!=0 and len(AMA2)!=0):
            ...
            if AMA1[-1]>AMA2[-1] and self.long == False :
                self.long = True
                print(self.etime,'open long positions%s %s %d %f' % (self.symbol[:4], self.symbol[5:], 8000,self.price))

                #self.long_price =
                self.open_long(self.symbol[:4], self.symbol[5:], price=self.price, volume=8000)
            if AMA1[-1]<AMA2[-1] and self.long:
                #  close clong
                self.long = False
                self.long_price = self.price
                print(self.etime,'close long positions%s %s %d %f' % (self.symbol[:4], self.symbol[5:], 8000,self.price))
                self.close_long(self.symbol[:4], self.symbol[5:], price=self.price, volume=8000)
